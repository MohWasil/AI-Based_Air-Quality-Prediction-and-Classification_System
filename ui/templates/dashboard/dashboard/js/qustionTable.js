...
function displayQustionTable(page) {
    ...
    tableBody.innerHTML = '';
    qustions.slice(startIndex, endIndex).forEach(q => {
        const row = `<tr>
                        <td>${q.id}</td>
                        <td>${q.name}</td>
                        <td>${q.phone}</td>
...
function setupQustionPagination() {
    const totalPages = Math.ceil(qustions.length / qustionRowsPerPage);
    const pagination = document.getElementById('qustionPagination');

    // Clear existing page links
    const pageLinks = Array.from(pagination.querySelectorAll('.page-item:not(#qustionPreviousPage):not(#qustionNextPage)'));
    pageLinks.forEach(link => link.remove());

    // Determine range of page numbers to display
    const maxVisiblePages = 4;
    let startPage = Math.max(1, qustionCurrentPage - Math.floor(maxVisiblePages / 2));
    ...
    for (let i = startPage; i <= endPage; i++) {
        ...
        pagination.insertBefore(pageItem, document.getElementById('qustionNextPage'));
    }

    // Disable/Enable Previous and Next buttons
    document.getElementById('qustionPreviousPage').classList.toggle('disabled', qustionCurrentPage === 1);
    document.getElementById('qustionNextPage').classList.toggle('disabled', qustionCurrentPage === totalPages);
