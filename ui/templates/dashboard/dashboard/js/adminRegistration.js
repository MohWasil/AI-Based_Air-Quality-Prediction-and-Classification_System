document.addEventListener('DOMContentLoaded', function () {
    ...
    password.addEventListener('input', function () {
        ...
        if (!/(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[!@#$%^&*]).{8,}/.test(password.value)) {
            errorElement.textContent = 'Password must be at least 8 characters long and include uppercase, lowercase, numbers, and symbols.';
        } else {
            errorElement.textContent = '';
        }
    });
    ...
    username.addEventListener('input', function () {
        ...
        if (!/^[a-zA-Z0-9]+$/.test(username.value)) {
            errorElement.textContent = 'Username can only include letters and numbers.';
        } else {
            errorElement.textContent = '';
        }
    });
    ...
    document.getElementById('registrationForm').addEventListener('submit', function (event) {
        ...
        if (!/(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[!@#$%^&*]).{8,}/.test(password.value)) {
            errors.password = 'Password must be at least 8 characters long and include uppercase, lowercase, numbers, and symbols.';
            isValid = false;
        }
        if (password.value !== confirmPassword.value) {
        ...
        if (isValid) {
            alertText.textContent = 'All your information is valid.';
            alertText.style.color = 'green';
        } else {
            alertText.textContent = 'Your Information is Invalid!';
