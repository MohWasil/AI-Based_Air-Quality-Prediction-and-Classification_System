...
function setupOrganizationPagination() {
  ...
  const pageLinks = Array.from(
    pagination.querySelectorAll(
      ".page-item:not(#org-previous-page):not(#org-next-page)"
    )
  );
  ...
  document
    .getElementById("org-previous-page")
    .classList.toggle("disabled", orgCurrentPage === 1);
  document
    .getElementById("org-next-page")
    .classList.toggle("disabled", orgCurrentPage === totalPages);
...
document.addEventListener("DOMContentLoaded", () => {
  fetch(
    "/api/accounts/organizational-users?format=json"
  ).then((response) =>
