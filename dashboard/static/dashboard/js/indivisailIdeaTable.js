...
function setupIndivisialPagination() {
  ...
  const pageLinks = Array.from(
    pagination.querySelectorAll(
      ".page-item:not(#indivisialPreviousPage):not(#indivisialNextPage)"
    )
  );
  ...
  let startPage = Math.max(
    1,
    indivisialCurrentPage - Math.floor(maxVisiblePages / 2)
  );
  ...
  document
    .getElementById("indivisialPreviousPage")
    .classList.toggle("disabled", indivisialCurrentPage === 1);
  document
    .getElementById("indivisialNextPage")
    .classList.toggle("disabled", indivisialCurrentPage === totalPages);
...
document.addEventListener("DOMContentLoaded", () => {
  fetch("http://localhost:8000/api/feedbacks/individual-ideas?format=json").then(
    (response) =>
      response.json().then((response) => {
        individualIdeas = response;
        displayIndivisialTable(indivisialCurrentPage);
        setupIndivisialPagination();
      })
  );
